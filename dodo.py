...
def task_clone_bioschemas():
    ...
    return {
        "actions": [
            "git clone https://github.com/BioSchemas/specifications.git --depth 1"
        ],
        "targets": [git_repo],
...
def task_rocrate_uris():
    ...
    def _task():
        module = module_from_context(
            requests.get("https://w3id.org/ro/crate/1.2-DRAFT/context").json()
        )
        add_docstring(module, "URIs for RO-Crate")
        dest_path.write_text(unparse(module))

    return {"actions": [_task], "targets": [dest_path], "uptodate": [True]}
