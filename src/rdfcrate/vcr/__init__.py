...
class ExternalFileSerializer:
    ...
    def serialize(self, cassette_dict: dict[str, Any]):
        if not isinstance(self.base_path, Path):
            raise ValueError(
                "When writing to external files, base_path must be a writeable path ie the package must be installed in editable mode."
            )
        ...
        for interaction in cassette_dict["interactions"]:
            ...
            if (
                ...
                and "string" in body
                and len(body["string"]) > self.min_size
            ):
                # Write the body to an external file
                file_path = (
                    self.base_path
                    / f"{interaction['request']['uri'].replace('/', '_').replace(':', '_')}.body"
                )
...
vcr.register_serializer(
    "externalfile", ExternalFileSerializer(base_path=HERE, min_size=1024)
)
...
def generate_cassettes():
    ...
    if not isinstance(CONTEXT_CASSETTE, Path):
        raise ValueError(
            "When regenerating cassettes, the package must be installed in editable mode."
        )
    ...
    with vcr.use_cassette(
        ...
        before_record_response=filter_headers,
    ) as cassette:
        urlopen(
